...
const RevisionPage = () => {
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Unable to load revision queue.');
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    <div className="space-y-6 pb-12 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Priority is based on elapsed time since the latest attempt and the difficulty of your last outcome.
            </p>
          </div>
          <div className="flex items-center space-x-3">
            ...
            >
              {showFormula ? 'Hide Formula' : 'How Prioritization Works'}
            </button>
          </div>
        </div>

        {/* Expandable Mathematical Model Explanation */}
        {showFormula && (
          <div className="mt-4 p-4 rounded-lg bg-slate-900/90 border border-slate-800 text-xs text-slate-300 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-slate-200 uppercase tracking-wider text-[11px] font-mono">
                Deterministic Spaced-Repetition Model
              </span>
              <span className="text-slate-500 font-mono text-[10px]">No AI / Pure Mathematical Formula</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 text-slate-400 text-[11px]">
              <div>
              ...
              <div>
                ...
                <ul className="list-disc list-inside mt-0.5 space-y-0.5 font-mono">
                  <li>Struggled: +2.0 (Urgent recall)</li>
                  <li>Revisit Needed: +1.0 (Moderate recall)</li>
                  <li>Solved: +0.0 (Standard cycle)</li>
                </ul>
              </div>
            </div>
            <p className="text-slate-500 text-[11px] pt-1 border-t border-slate-800/80">
              Problems with no practice attempts are excluded. Items with higher scores require sooner practice.
            </p>
          </div>
        )}
      </div>
      ...
      {!isLoading && !error && queue.length === 0 && (
        <div className="bg-slate-900 border border-dashed border-slate-800 rounded-xl p-12 text-center max-w-lg mx-auto">
          <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center mx-auto mb-3 text-slate-400 font-mono text-sm">
            0
          </div>
          <h2 className="text-base font-semibold text-slate-200">No problems due for revision</h2>
          <p className="text-xs text-slate-400 mt-1 leading-relaxed">
            Your revision queue is empty. As you solve problems and log practice sessions, problems will be scheduled here according to their spaced repetition intervals.
          </p>
          <div className="mt-5">
            <Link
              to="/problems"
              className="inline-flex items-center px-3.5 py-1.5 rounded-md text-xs font-semibold bg-emerald-400 text-slate-950 hover:bg-emerald-300 transition-colors"
            >
      ...
      {!isLoading && !error && queue.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs text-slate-400 px-1">
            <span>
              Showing <strong className="text-white font-mono">{queue.length}</strong> prioritized items
            </span>
            <span className="font-mono text-[11px]">Ranked by priority score descending</span>
          </div>
          ...
          <div className="space-y-2.5">
            {queue.map((item, index) => {
              ...
              return (
                <div
                  key={item.problemId}
                  className="bg-slate-900 border border-slate-800 hover:border-slate-700 transition-colors rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-4 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="px-2.5 py-1 rounded bg-slate-950 border border-slate-800 text-center font-mono">
                        <span className="text-[10px] uppercase text-slate-400 block leading-tight">Score</span>
                      ...
                      <Link
                        to={`/problems/${item.problemId}`}
                        className="px-3 py-1.5 text-xs font-semibold rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition-colors"
                      >
                        Solve &rarr;
                      </Link>
                    </div>
